...
class TaskMGR: #це основа робота планувальника
    def __init__(self):
        ...
        try:
            ...
            a = open("task.txt" , "w" , encoding="utf-8") # це створення + заміна
    ...
    def print_task(self): #це вивід
        ...
        for index , i in enumerate(self.tasks):  
            try:
                ...
                t = (normalDedline - today).days #це 'Дельта дат'
                if t <= 0:
                ...
                color = ""
            print(f"{color}{index}: {i.name} | {i.dedline} | {i.deskr}{colorama.Style.RESET_ALL}") #це сам вивід з форматуванням
...
elif a not in ("1", "2"): #це швидкий набір по типу Name+1description
    mat = re.match(r"([^\+\d]+)\+(\d+)(.*)", a) #це розподіл
    b = mat.group(1).strip() #назва
    try:
